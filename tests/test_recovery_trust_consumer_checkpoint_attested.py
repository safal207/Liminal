...
GENERATION_2_MANIFEST_KEY = "drill/generation-2-manifest.json"
...
def test_same_generation_conflict_rejected() -> None:
    checkpoint = _load(CHECKPOINT_PATH)
    manifest_0 = _load(MANIFEST_PATH)
    conflict_manifest = copy.deepcopy(manifest_0)
    roots = conflict_manifest["roots"]
    assert isinstance(roots, dict)
    verifier = roots["verifier"]
    assert isinstance(verifier, dict)
    verifier["workflow_sha"] = "f" * 40
    conflict_digest = sha256_hex(canonical_json_bytes(conflict_manifest))
    conflict_path = "drill/conflicting-generation-0-manifest.json"
    conflicting_registry = {
        "schema_version": REGISTRY_SCHEMA_VERSION,
        "active_generation": 0,
        "active_manifest_sha256": conflict_digest,
        "history": [
            {
                "generation": 0,
                "manifest_path": conflict_path,
                "manifest_sha256": conflict_digest,
            }
        ],
    }
    ...
    decision = evaluate_candidate(
        checkpoint,
        conflicting_registry,
        {conflict_path: conflict_manifest},
        advance_evidence=None,
    )
    assert not decision.authorized
    assert decision.reason == "same_generation_registry_conflict"


def test_generation_gap_rejected() -> None:
    checkpoint, registry_1, manifests = _candidate()
    manifest_1 = manifests[GENERATION_1_MANIFEST_KEY]
    assert isinstance(manifest_1, dict)
    manifest_2 = copy.deepcopy(manifest_1)
    manifest_2["generation"] = 2
    manifest_2["previous_manifest_sha256"] = registry_1["active_manifest_sha256"]
    roots = manifest_2["roots"]
    assert isinstance(roots, dict)
    verifier = roots["verifier"]
    assert isinstance(verifier, dict)
    verifier["workflow_sha"] = "d" * 40
    manifest_2_sha256 = sha256_hex(canonical_json_bytes(manifest_2))
    history = registry_1["history"]
    assert isinstance(history, list)
    registry_2 = {
        "schema_version": REGISTRY_SCHEMA_VERSION,
        "active_generation": 2,
        "active_manifest_sha256": manifest_2_sha256,
        "history": [
            *history,
            {
                "generation": 2,
                "manifest_path": GENERATION_2_MANIFEST_KEY,
                "manifest_sha256": manifest_2_sha256,
            },
        ],
    }
    manifests_2 = {**manifests, GENERATION_2_MANIFEST_KEY: manifest_2}
