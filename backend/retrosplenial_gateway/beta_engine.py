...
class BetaWaveEngine:
    ...
    def select_beta_state(
        self, content: str, urgency_level: float, cognitive_demand: float
    ) -> BetaState:
        ...
        processing_score = sum(
            1 for keyword in processing_keywords if keyword in content_lower
        )
        concentration_score = sum(
            1 for keyword in concentration_keywords if keyword in content_lower
        )
        control_score = sum(
            1 for keyword in control_keywords if keyword in content_lower
        )
    ...
    def generate_beta_frequency(
        self, beta_state: BetaState, urgency_level: float, cognitive_demand: float
    ) -> float:
        ...
        final_frequency = (
            base_freq + urgency_adjustment + demand_adjustment + variability
        )
    ...
    def calculate_control_strength(
        self, beta_state: BetaState, frequency: float, urgency_level: float
    ) -> float:
    ...
    def calculate_attention_intensity(
        self, beta_state: BetaState, amplitude: float, synchronization: float
    ) -> float:
    ...
    def generate_beta_oscillation(
        ...
        # Generate frequency
        frequency = self.generate_beta_frequency(
            beta_state, urgency_level, cognitive_demand
        )
        ...
        control_strength = self.calculate_control_strength(
            beta_state, frequency, urgency_level
        )

        # Calculate attention intensity
        attention_intensity = self.calculate_attention_intensity(
            beta_state, amplitude, synchronization
        )
    ...
    def get_cognitive_enhancement_factor(self) -> float:
        ...
        attention_enhancement = 1.0 + (
            self.current_oscillation.attention_intensity * 0.5
        )
        ...
        total_enhancement = (
            attention_enhancement + control_enhancement + sync_enhancement
        ) / 3
    ...
    def is_high_performance_state(self) -> bool:
        ...
        return (
            self.current_oscillation.attention_intensity > 0.8
            and self.current_oscillation.control_strength > 0.7
            and self.current_oscillation.synchronization > 0.6
        )
    ...
    def is_executive_control_active(self) -> bool:
        ...
        return (
            self.current_oscillation.beta_state == BetaState.EXECUTIVE_CONTROL
            and self.current_oscillation.control_strength > 0.7
        )
    ...
    def get_beta_analytics(self) -> Dict[str, Any]:
        ...
        # Calculate averages
        avg_sync = (
            sum(self.synchronization_history) / len(self.synchronization_history)
            if self.synchronization_history
            else 0
        )
        avg_control = (
            sum(self.control_events) / len(self.control_events)
            if self.control_events
            else 0
        )
        recent_attention_episodes = len(
            [
                ep
                for ep in self.attention_episodes
                if time.time() - ep["timestamp"] < 600
            ]
        )  # Last 10 minutes
        ...
        return {
            "current_state": {
                ...
                "attention_intensity": round(
                    self.current_oscillation.attention_intensity, 3
                ),
            },
            "analytics": {
                "cognitive_enhancement": round(
                    self.get_cognitive_enhancement_factor(), 3
                ),
            ...
            "optimization": {
                "focus_potential": round(
                    self.current_oscillation.attention_intensity, 3
                ),
                "control_readiness": round(
                    self.current_oscillation.control_strength, 3
                ),
                "cognitive_synchrony": round(
                    self.current_oscillation.synchronization, 3
                ),
            },
        }
