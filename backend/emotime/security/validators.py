    ...
    def validate_module_import(
        ...
        try:
            ...
            if module_name not in self.allowed_modules:
                violations.append(
                    SecurityViolation(
                        ...
                        data={
                            "module": module_name,
                            "allowed": list(self.allowed_modules),
                        },
                    )
                )
            ...
            if critical_violations:
                ...
                raise SecurityError(
                    f"Critical security violations: {'; '.join(violation_msgs)}"
                )
            ...
            for violation in violations:
                if violation.severity != "critical":
                    safe_logger.warning(
                        f"Security violation ({violation.severity}): {violation.message}"
                    )
...
class InputSanitizer:
    ...
    def sanitize_text(self, text: str) -> str:
        ...
        if len(text) > self.max_text_length:
            safe_logger.warning(
                f"Text input truncated from {len(text)} to {self.max_text_length} chars"
            )
