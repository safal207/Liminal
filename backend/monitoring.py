...
class HealthMonitor:
    ...
    def get_overall_health(self) -> Dict[str, Any]:
        ...
        if critical_unhealthy:
            ...
            status == HealthStatus.DEGRADED for status in self.health_status.values()
        ):
            overall_status = HealthStatus.DEGRADED
        elif any(
            status == HealthStatus.UNKNOWN for status in self.health_status.values()
        ):
...
class AlertManager:
    ...
    async def fire_alert(
        self, title: str, description: str, severity: AlertSeverity, source: str, **tags
    ) -> str:
        """Fire an alert."""
        ...
        logger.warning(
            f"Alert fired: {title}",
            extra={"alert_id": alert_id, "severity": severity.value, "source": source},
        )
...
class MonitoringService:
    ...
    def _register_default_alerts(self):
        ...
        def circuit_breaker_open():
            ...
            health = resilience_manager.get_health_status()
            return any(
                cb["state"] == "open" for cb in health["circuit_breakers"].values()
            )
    ...
    async def get_monitoring_dashboard(self) -> Dict[str, Any]:
        ...
        active_alerts = [
            alert for alert in self.alert_manager.alerts.values() if not alert.resolved
        ]
...
def record_business_metric(
    name: str, value: Union[int, float], unit: str, description: str = None, **tags
):
...
async def record_emotion_entry(emotion: str, intensity: float):
    ...
    monitoring_service.metrics.emotion_entries.labels(
        emotion=emotion, intensity_range=intensity_range
    ).inc()
...
if __name__ == "__main__":
    ...
    async def test_monitoring():
        ...
        await monitoring_service.alert_manager.fire_alert(
            "Test Alert", "This is a test alert", AlertSeverity.WARNING, "test_system"
        )
