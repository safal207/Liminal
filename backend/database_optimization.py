...
class ConnectionPool:
    ...
    async def initialize(self, connection_factory):
        ...
        logger.info(
            f"Initialized {self.db_type.value} connection pool with {len(self.connections)} connections"
        )

    async def _create_connection(
        self, replica_type: str = "primary"
    ) -> DatabaseConnection:
        ...
        try:
            ...
            logger.debug(
                f"Created new {self.db_type.value} connection: {connection_id}"
            )
    ...
    async def acquire_connection(
        self, query_type: QueryType = QueryType.READ
    ) -> DatabaseConnection:
        ...
        async with self._lock:
            ...
            if not candidate_pool:
                ...
                if len(self.connections) < self.max_connections:
                    replica_type = (
                        "read_replica" if query_type == QueryType.READ else "primary"
                    )
...
class QueryOptimizer:
    ...
    async def _execute_raw_query(
        self, query: str, params: Dict, connection: DatabaseConnection
    ) -> Any:
        ...
        if connection.db_type == DatabaseType.NEO4J:
            ...
            return await connection.connection.fetch(
                query, *(params.values() if params else [])
            )

        elif connection.db_type == DatabaseType.REDIS:
            # Redis operations would be different
            return await connection.connection.execute_command(
                query, *params.values() if params else []
            )
...
class DatabaseManager:
    ...
    async def initialize(self):
        ...
        if self.settings.neo4j_uri:
            neo4j_pool = ConnectionPool(
                DatabaseType.NEO4J, min_connections=3, max_connections=15
            )
        ...
        if self.settings.redis_enabled:
            redis_pool = ConnectionPool(
                DatabaseType.REDIS, min_connections=5, max_connections=20
            )
    ...
    async def _create_neo4j_connection(
        self, replica_type: str = "primary"
    ) -> AsyncDriver:
    ...
    async def get_connection(
        self, db_type: DatabaseType, query_type: QueryType = QueryType.READ
    ):
        ...
        async with self.get_connection(db_type, query_type) as connection:
            ...
            try:
                ...
                monitoring_service.metrics.db_operations_total.labels(
                    database=db_type.value, operation=query_type.value, status="success"
                ).inc()

                monitoring_service.metrics.db_query_duration.labels(
                    database=db_type.value, operation=query_type.value
                ...
                monitoring_service.metrics.db_operations_total.labels(
                    database=db_type.value, operation=query_type.value, status="error"
                ).inc()
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        for db_type, pool in self.pools.items():
            try:
                ...
                health_status[db_type.value] = {"status": "unhealthy", "error": str(e)}
...
async def execute_neo4j_query(
    query: str, params: Dict = None, read_only: bool = True
) -> Any:
    """Execute Neo4j query with optimization."""
    query_type = QueryType.READ if read_only else QueryType.WRITE
    return await db_manager.execute_query(DatabaseType.NEO4J, query, params, query_type)
