...
class TestEmotimeEngine:
    ...
    async def test_process_sensor_data(self):
        ...
        sensor_data = SensorData(
            ...
            raw_data=TextData(text="I feel great today!", word_count=4, char_count=20),
            metadata={},
        )
        ...
        with patch.object(
            ...
            with patch.object(self.engine.modes, "classify_mode") as mock_classify:
                ...
                sensor_data = SensorData(
                    ...
                    timestamp=datetime.now(),
                    raw_data=TextData(
                        text="I'm concentrating hard", word_count=3, char_count=22
                    ),
                    metadata={},
                )
                await self.engine.process_sensor_data(sensor_data)
    ...
    async def test_emotional_insights(self):
        ...
        with patch.object(
            self.engine.timeseries, "to_dict", return_value={"points": 10}
        ):
            with patch.object(
                self.engine.modes, "get_mode_statistics", return_value={"calm": 0.3}
            ):
                with patch.object(
                    self.engine.modes,
                    "get_mode_insights",
                    return_value={"dominant_mode": "calm"},
                ):
                    with patch.object(
                        self.engine.fusion,
                        "get_feature_statistics",
                        return_value={"avg_valence": 0.6},
                    ):
