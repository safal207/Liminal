    ...
    async def test_websocket_emotime_flow(self):
        ...
        try:
            ...
            async with websockets.connect(self.ws_url) as websocket:
                ...
                auth_message = {"token": self.jwt_token}
                ...
                for i, message_text in enumerate(emotional_messages):
                    ...
                    print(
                        f"   → Sending emotional message {i+1}: {message_text[:50]}..."
                    )
                    ...
                    try:
                        ...
                        if response_data.get("type") == "home_state_update":
                            print(
                                f"     Home state detected! Authenticity: {response_data.get('data', {}).get('authenticity_level', 'unknown')}"
                            )
                ...
                try:
                    response = requests.get(
                        f"{self.http_url}/emotime/status", timeout=5
                    )

                    if response.status_code == 200:
                        emotime_status = response.json()
                        print(
                            f"   ✓ Emotime status: {emotime_status.get('status', 'unknown')}"
                        )
                        ...
                        if emotime_status.get("status") == "active":
                            ...
                            print(
                                f"     Valence: {features.get('valence', 'unknown'):.2f}"
                            )
                            print(
                                f"     Confidence: {emotime_status.get('confidence', 'unknown'):.2f}"
                            )
                ...
                try:
                    metrics_response = requests.get(
                        f"{self.http_url}/metrics", timeout=5
                    )
                    ...
                    if metrics_response.status_code == 200:
                        ...
                        found_ws = sum(
                            1 for metric in ws_metrics if metric in metrics_text
                        )
                        found_emotime = sum(
                            1 for metric in emotime_metrics if metric in metrics_text
                        )

                        print(
                            f"   ✓ WebSocket metrics: {found_ws}/{len(ws_metrics)} found"
                        )
                        print(
                            f"   ✓ Emotime metrics: {found_emotime}/{len(emotime_metrics)} found"
                        )
    ...
    async def test_emotime_session_management(self):
        ...
        try:
            ...
            response = requests.post(
                f"{self.http_url}/emotime/session/start", json=start_payload, timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                response = requests.post(
                    f"{self.http_url}/emotime/text", json=text_payload, timeout=10
                )
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"   ✓ Timeline retrieved: {len(timeline.get('timeline', []))} points"
                    )

                # Stop session
                stop_payload = {"user_id": self.test_user_id, "session_id": session_id}
