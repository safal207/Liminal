...
if FASTAPI_AVAILABLE:
    ...
    async def websocket_emotime_endpoint(
        ...
        try:
            ...
            if token:
                try:
                    ...
                    safe_logger.warning(
                        f"WebSocket authentication failed: {auth_error}"
                    )
                ...
                safe_logger.warning(
                    f"WebSocket connection without authentication for user {user_id}"
                )
        ...
        if message_type == "sensor_data":
            ...
            if room_id:
                ...
                for (
                    conn_id,
                    metrics,
                ) in connection_manager.pool.connection_metrics.items():
