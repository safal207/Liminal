...
class BurnoutDetector:
    ...
    def _analyze_patterns(self, history: List[Any]) -> float:
        ...
        recent_modes = [getattr(d, 'mode', getattr(d, 'type', None)) for d in history[-5:]]
    ...
    def _analyze_valence_risk(self, history: List[Any]) -> float:
        ...
        if avg_valence < -0.5: return 0.8
        elif avg_valence < -0.2: return 0.5
        return 0.0

    def _generate_indicators(self, history: List[Any], mode_counts: Dict, risk_score: float) -> List[str]:
