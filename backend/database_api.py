...
class DataStoreRequest(BaseModel):
    ...
    data_type: str = Field(
        ..., description="Тип данных (emotion_history, philosophy, etc.)"
    )
...
async def store_data(
    request: DataStoreRequest, adapter: DatabaseAdapter = Depends(get_database_adapter)
):
    """
...
async def query_data(
    request: DataQueryRequest, adapter: DatabaseAdapter = Depends(get_database_adapter)
):
    """
    ...
    try:
        ...
        data = await adapter.query_data(
            data_type=data_type, filters=request.filters, limit=request.limit
        )
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка запроса данных: {str(e)}")


@router.get("/health", response_model=HealthResponse)
async def get_database_health(adapter: DatabaseAdapter = Depends(get_database_adapter)):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка получения статуса БД: {str(e)}"
        )
...
async def get_api_examples():
    ...
    return {
        ...
        "health_check": {"method": "GET", "url": "/api/database/health"},
    }
