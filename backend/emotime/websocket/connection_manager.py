...
class ConnectionPool:
    ...
    def get_connection_stats(self) -> Dict[str, Any]:
        ...
        if self.connection_metrics:
            avg_quality = sum(
                m.quality_score for m in self.connection_metrics.values()
            ) / len(self.connection_metrics)
            total_messages = sum(
...
class EmotimeConnectionManager:
    ...
    def __init__(self, max_connections: int = 1000):
        ...
        self.emotion_subscribers: Dict[str, List[str]] = defaultdict(
            list
        )  # user_id -> [connection_ids]
        self.room_subscribers: Dict[str, List[str]] = defaultdict(
            list
        ...
        if not WEBSOCKET_AVAILABLE:
            safe_logger.warning(
                "WebSocket not available - connection manager in mock mode"
            )
    ...
    async def connect_user(self, websocket: WebSocket, user_id: str) -> str:
        ...
        try:
            ...
            await self._trigger_event(
                "user_connected", {"connection_id": connection_id, "user_id": user_id}
            )
    ...
    async def join_room(self, connection_id: str, room_id: str):
        ...
        if connection_id not in self.room_subscribers[room_id]:
            ...
            await self._trigger_event(
                "room_joined", {"connection_id": connection_id, "room_id": room_id}
            )
    ...
    async def leave_room(self, connection_id: str, room_id: str):
        ...
        if connection_id in self.room_subscribers[room_id]:
            ...
            await self._trigger_event(
                "room_left", {"connection_id": connection_id, "room_id": room_id}
            )
        ...
        for connection_id in connection_ids:
            ...
            if websocket and websocket.client_state == WebSocketState.CONNECTED:
                try:
                    ...
                    safe_logger.warning(
                        f"Failed to send to room {room_id}, connection {connection_id}: {e}"
                    )
                    failed_connections.append(connection_id)
            else:
                failed_connections.append(connection_id)
