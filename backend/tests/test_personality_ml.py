...
class TestPersonalityAdapterML:
    ...
    async def test_analyze_text_emotion(self, adapter):
        ...
        result = await adapter.analyze_text_emotion(
            "Я очень доволен результатами тестирования!", "unit-test"
        )
    ...
    async def test_analyze_text_emotion_with_error(self, adapter):
        ...
        # Анализируем текст
        result = await adapter.analyze_text_emotion("Проблемный текст", "error-test")
    ...
    async def test_get_emotion_suggestions_with_error(self, adapter):
        ...
        # Настраиваем мок для вызова исключения
        adapter.ml_adapter.get_emotion_suggestions.side_effect = Exception(
            "Тестовая ошибка"
        )
...
class TestMLIntegration:
    ...
    async def test_full_ml_pipeline(self):
        ...
        with patch(
            "backend.database_adapter.DatabaseAdapter.store_data",
            new_callable=AsyncMock,
        ) as mock_store:
            # Анализируем текст
            result = await personality_adapter.analyze_text_emotion(
                "Я очень рад, что интеграционные тесты работают!", "integration-test"
            )
            ...
            suggestions = await personality_adapter.get_emotion_suggestions(
                "integration-test"
            )
