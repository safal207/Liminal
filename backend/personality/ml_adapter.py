...
class EmotionMLAdapter:
    ...
    def __init__(self):
        ...
        logger.info(
            "EmotionMLAdapter initialized with caching, async and multilingual support"
        )
    ...
    async def analyze_multilingual(
        self, text: str, target_lang: str = "ru", expected_lang: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        if target_lang not in self.supported_languages:
            logger.warning(
                f"Неподдерживаемый язык: {target_lang}, используется русский по умолчанию"
            )
            target_lang = "ru"

        # Используем функцию из модуля мультиязычной поддержки
        result = await analyze_multilingual_text(
            text, self.analyze_text, target_lang, expected_lang=expected_lang
        )
    ...
    async def analyze_text_stream(
        self, text_stream: Union[List[str], asyncio.Queue]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _map_dimensions_to_emotion(
            ...
            try:
                ...
                for emotion in base_emotions:
                    distance = (
                        sum(
                            (
                                analysis["dimensions"].get(dim, 0.5)
                                - emotion.get(dim, 0.5)
                            )
                            ** 2
                    ...
                    emotion["relevance"] = 1.0 - min(
                        distance, 1.0
                    )  # Чем меньше расстояние, тем выше релевантность

                # Сортируем по релевантности
                sorted_emotions = sorted(
                    base_emotions, key=lambda e: e.get("relevance", 0.0), reverse=True
                )
...
async def analyze_large_text(
    ...
    logger.info(
        f"Analyzing large text: {len(text)} chars split into {len(chunks)} chunks"
    )
