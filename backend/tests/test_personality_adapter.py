...
class TestPersonalityAdapter:
    ...
    def adapter_with_mocks(self, mock_database_adapter):
        ...
        with patch(
            "backend.personality.adapter.DatabaseAdapter",
            return_value=mock_database_adapter,
        ):
    ...
    async def test_store_emotion(self, adapter_with_mocks):
        ...
        emotion = await adapter.store_emotion(
            emotion_type="спокойствие", intensity=0.7, context="тестирование метода"
        )
    ...
    async def test_update_preference(self, adapter_with_mocks):
        ...
        preference = await adapter.update_preference(
            category="музыка", value="классическая", strength=0.85
        )
    ...
    async def test_get_recommendations(self, adapter_with_mocks):
        ...
        recommendations = await adapter.get_recommendations(limit=2, context="обучение")
...
class TestPersonalitySchema:
    ...
    def test_personality_profile_type(self):
        ...
        preferences = [
            Preference(
                category="музыка", value="классическая", strength=0.7, last_updated=now
            ),
            Preference(
                category="обучение", value="визуальный", strength=0.9, last_updated=now
            ),
            Preference(
                category="еда", value="вегетарианская", strength=0.5, last_updated=now
            ),
        ]
...
class TestPersonalityAdapterIntegration:
    ...
    async def test_real_database_integration(self):
        ...
        try:
            ...
            pytest.skip(
                f"Реальные БД недоступны для интеграционного тестирования: {str(e)}"
            )
