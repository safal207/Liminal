    ...
    def generate_token(
        ...
        try:
            ...
            if not self._check_rate_limit(user_id):
                raise SecurityError(
                    f"Token generation rate limit exceeded for user {user_id}"
                )
            ...
            safe_logger.info(
                f"JWT token generated for user {user_id}, type: {token_type.value}"
            )
    ...
    def _check_rate_limit(self, user_id: str) -> bool:
        ...
        if user_id in self.token_generation_history:
            self.token_generation_history[user_id] = [
                timestamp
                for timestamp in self.token_generation_history[user_id]
...
def generate_secure_token(
    user_id: str, token_type: TokenType = TokenType.WEBSOCKET, **kwargs
) -> str:
    """Generates a secure JWT token."""
