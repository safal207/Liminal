...
class MessageQueue:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            priority.name.lower(): len(queue) for priority, queue in self.queues.items()
        }
...
class ConnectionPool:
    ...
    async def get_connections_for_subscription(
    ...
    async def _store_connection_in_redis(self, connection_info: ConnectionInfo):
        ...
        try:
            ...
            await self.redis_client.expire(
                f"ws_connection:{connection_info.connection_id}", 3600  # 1 hour
            )
...
class WebSocketLoadBalancer:
    ...
    async def register_server(self) -> bool:
        ...
        try:
            ...
            await self.redis_client.hset(
                f"ws_server:{self.server_id}", mapping=server_info
            )

            await self.redis_client.expire(
                f"ws_server:{self.server_id}", self.heartbeat_interval * 3
            )
    ...
    async def update_server_load(self, current_load: int):
        ...
        try:
            await self.redis_client.hset(
                f"ws_server:{self.server_id}", "current_load", str(current_load)
            )
        except Exception as e:
    ...
    async def _heartbeat_loop(self):
        ...
        while True:
            try:
                ...
                await self.redis_client.hset(
                    f"ws_server:{self.server_id}", "last_heartbeat", str(time.time())
                )
            except Exception as e:
...
class WebSocketManager:
    ...
    async def broadcast_to_subscription(
        ...
        connections = await self.connection_pool.get_connections_for_subscription(
            subscription
        )
    ...
    async def _handle_consciousness(
