...
class BurnoutPermissionChecker:
    ...
    @classmethod
    def has_permission(
        cls, user_context: BurnoutUserContext, permission: BurnoutPermission
    ) -> bool:
    ...
    def can_access_team_data(
        cls, user_context: BurnoutUserContext, target_team_id: str
    ) -> bool:
        ...
        if user_context.team_id == target_team_id:
            return cls.has_permission(
                user_context, BurnoutPermission.VIEW_TEAM_ANALYTICS
            )
    ...
    @classmethod
    def can_manage_user(
        cls, user_context: BurnoutUserContext, target_user_id: str
    ) -> bool:
...
class BurnoutAuthService:
    ...
    async def get_burnout_user_context(
        ...
        is_hr_manager = (
            BurnoutRole.HR_MANAGER.value in roles
            or "hr_manager" in roles
            or "burnout:access_hr_dashboard" in scopes
        )

        is_team_lead = (
            BurnoutRole.TEAM_LEAD.value in roles
            or "team_lead" in roles
            or "burnout:view_team" in scopes
        )
...
def require_burnout_permission(permission: BurnoutPermission):
    ...
    async def check_permission(
        user_context: BurnoutUserContext = Depends(
            BurnoutAuthService.get_burnout_user_context
        ),
    ):
...
async def require_hr_access(
    ...
    if not BurnoutPermissionChecker.has_permission(
        user_context, BurnoutPermission.ACCESS_HR_DASHBOARD
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="HR manager access required"
        )
...
class BurnoutAuditLogger:
    ...
    async def log_critical_access(
        user_id: str, team_id: str, risk_level: str, operation: str
    ):
    ...
    async def log_data_export(
        user_id: str, export_type: str, team_ids: List[str], record_count: int
    ):
