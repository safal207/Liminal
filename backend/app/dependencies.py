...
def reset_neo4j_gateway() -> None:
    ...
    if _timeline_bridge_listener is not None:
        _container().memory_timeline_service().remove_listener(_timeline_bridge_listener)
        _timeline_bridge_listener = None
...
def get_memory_service() -> MemoryTimelineService:
    ...
    return _container().memory_timeline_service()


def get_memory_timeline():
    """Return the shared memory timeline instance."""
    return get_memory_service().get_timeline()
...
def get_connection_manager_service() -> ConnectionManagerService:
    ...
    return _container().connection_manager_service()
...
def get_websocket_service() -> TimelineWebSocketService:
    ...
    if _websocket_service is None:
        _websocket_service = TimelineWebSocketService(
            _container().connection_manager_service(),
            _container().memory_timeline_service(),
            _container().ml_service(),
            get_auth_service(),
        )
