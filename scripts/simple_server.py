...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* Sales trigger chips (Samcart-like) */
    ...
    <script>
        ...
        function toggleSimpleMode() {
            try {
                ...
                if (on) { body.classList.add('simple'); sw.classList.add('on'); }
                else { body.classList.remove('simple'); sw.classList.remove('on'); }
            } catch (error) {
            ...
            try {
                ...
                tbody.innerHTML = data.topAtRiskEdges.map(edge => {
                    ...
                    return `<tr class="${isRisk ? 'risk' : ''}">
                        ...
                        <td>${advice}${selected.length ? "<div style='margin-top:6px;display:flex;flex-wrap:wrap;gap:6px;'>" + selected.map(id => "<span class='chip'>#" + id + "</span>").join('') + "</div>" : ''}</td>
                    </tr>`;
                }).join('');
            } catch (error) {
        ...
        async function addNodeSimple() {
            try {
                ...
                // Simple picker via prompt (no extra libs)
        ...
        // Простой UI self-test без внешних зависимостей
        ...
        // Функции уже экспортированы в window сразу после определения
    </script>
</body>
</html>"""
