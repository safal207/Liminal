...
class DirectionalSpace:
    ...
    def blend_with(
        self, other: "DirectionalSpace", weight: float = 0.5
    ) -> "DirectionalSpace":
        ...
        return DirectionalSpace(
            ...
            north_strength=self.north_strength * (1 - weight)
            + other.north_strength * weight,
            south_strength=self.south_strength * (1 - weight)
            + other.south_strength * weight,
            east_strength=self.east_strength * (1 - weight)
            + other.east_strength * weight,
            west_strength=self.west_strength * (1 - weight)
            + other.west_strength * weight,
...
class NavigationContext:
    ...
    def calculate_context_score(self) -> float:
        ...
        return (
            self.context_clarity + self.context_stability + self.context_relevance
        ) / 3
...
class DirectionalAnalyzer:
    ...
    def analyze_directional_pattern(
        self, directions: List[SemanticDirection]
    ) -> Dict[str, Any]:
        ...
        pattern_analysis = {
            ...
            "dominant_percentage": (
                most_common[0][1] / len(directions) if most_common else 0.0
            ),
        }
        ...
        pattern_analysis["confidence"] = self._calculate_pattern_confidence(
            directions, pattern_type
        )
    ...
    def _calculate_pattern_confidence(
        self, directions: List[SemanticDirection], pattern_type: str
    ) -> float:
