...
class LIMINALPerformanceProfiler:
    ...
    async def _analyze_core_endpoints(self):
        ...
        endpoints = ["/health", "/metrics", "/emotime/status", "/emotime/health"]
    ...
    async def _analyze_emotime_performance(self):
        ...
        test_cases = [
            ...
            {
                "text": "Medium length emotional text with some complexity",
                "expected_time": 1000,
            },
            {
                "text": " ".join(["Very long emotional text"] * 20),
                "expected_time": 2000,
            },
        ]
    ...
    async def _measure_endpoint_performance(
        self, endpoint: str, samples: int = 10
    ) -> PerformanceMetric:
    ...
    def _generate_recommendations(self):
        ...
        if health_metric and health_metric.avg_response_time > 1000:
            recommendations.append(
                "Health endpoint is slow - check database connections and reduce startup checks"
            )
        ...
        if slow_emotime:
            recommendations.append(
                "Emotime processing is slow - implement caching and batch processing"
            )
        ...
        if throughput_10.get("throughput", 0) < 5:
            recommendations.append(
                "Low throughput detected - consider async optimization and connection pooling"
            )
        ...
        if stress.get("success_rate", 100) < 95:
            recommendations.append(
                "High error rate under stress - improve error handling and resource limits"
            )
