...
def test_wrapper_pins_v0_3_builder_and_hash_locked_verifier() -> None:
    ...
    assert "Authorize embedded builder environment" in workflow
    assert "check-builder-environment-authorization.py" in workflow
    assert "policies/trusted-recovery-proof-builder-v0.3.json" in workflow
    assert "builder-environment-authorization.json" in workflow
