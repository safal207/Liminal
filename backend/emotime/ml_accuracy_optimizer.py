...
class EnhancedEmotionalClassifier:
    ...
    def _extract_lexical_features(self, text: str, text_lower: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "exclamation_ratio": (
                text.count("!") / len(text.split()) if text.split() else 0
            ),
            "emotional_intensifiers": sum(
                1
                for word in ["очень", "крайне", "невероятно", "чрезвычайно"]
    ...
    def _extract_syntactic_features(self, text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "complex_sentence_indicators": sum(
                1
                for word in ["но", "однако", "хотя", "несмотря"]
    ...
    def _extract_semantic_features(self, text_lower: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "emotional_diversity": len(
                [k for k, v in pattern_matches.items() if v > 0]
            ),
        }
    ...
    def _extract_contextual_features(self, text: str) -> Dict[str, Any]:
        ...
        text_lower = text.lower()
        return {
            "first_person_indicators": sum(
                1
                for word in ["я", "мне", "мой", "моя", "мое"]
                if word in text_lower.split()
            ),
            "certainty_indicators": sum(
                1
                for phrase in ["я уверен", "точно знаю", "определенно"]
                if phrase in text_lower
            ),
            "uncertainty_indicators": sum(
                1
                for phrase in ["возможно", "может быть", "не знаю", "кажется"]
                if phrase in text_lower
            ),
            "temporal_references": sum(
                1
                for word in ["сейчас", "сегодня", "вчера", "завтра"]
                if word in text_lower
            ),
        }
    ...
    def _extract_temporal_features(self, text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "text_creation_speed_estimate": min(
                len(text) / 10, 1.0
            ),  # Примерная оценка скорости печати
        }
    ...
    def _multi_level_classification(
        ...
        for dim in EmotionalDimension:
            final_dimensions[dim.value] = (
                ...
                + semantic_scores.get(dim.value, 0.5)
                * self.calibration_weights["semantic"]
                + contextual_scores.get(dim.value, 0.5)
                * self.calibration_weights["contextual"]
                + syntactic_scores.get(dim.value, 0.5)
                * self.calibration_weights["syntactic"]
            )
    ...
    def _lexical_classification(
        self, text_lower: str, lexical_features: Dict
    ) -> Dict[str, float]:
        ...
        for pattern_name, pattern_data in self.emotional_patterns.items():
            matches = sum(
                1 for keyword in pattern_data["keywords"] if keyword in text_lower
            )

            if matches > 0:
                weight = min(matches / len(pattern_data["keywords"]), 1.0)
                dimensions["valence"] = (
                    dimensions["valence"] + pattern_data["valence"] * weight
                ) / 2
                dimensions["arousal"] = (
                    dimensions["arousal"] + pattern_data["arousal"] * weight
                ) / 2
                dimensions["dominance"] = (
                    dimensions["dominance"] + pattern_data["dominance"] * weight
                ) / 2
    ...
    def _calculate_prediction_confidence(self, features: Dict[str, Any]) -> float:
        ...
        if features["semantic"]["total_emotional_intensity"] > 0:
            confidence += min(
                features["semantic"]["total_emotional_intensity"] / 10.0, 0.2
            )
    ...
    def _estimate_prediction_accuracy(
        self, features: Dict[str, Any], dimensions: Dict[str, float]
    ) -> float:
        ...
        emotional_quality = min(
            features["semantic"]["total_emotional_intensity"] / 5.0, 1.0
        )
