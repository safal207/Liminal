...
class LiminalQualityFramework:
    ...
    async def _assess_performance(self) -> List[QualityMetric]:
        ...
        try:
            ...
            status = (
                "PASS"
                if response_time < self.quality_thresholds["response_time_ms"]
                else "FAIL"
            )
            ...
            status = (
                "PASS"
                if throughput > self.quality_thresholds["throughput_mps"]
                else "FAIL"
            )
            ...
            status = (
                "PASS"
                if error_rate < self.quality_thresholds["error_rate_percent"]
                else "FAIL"
            )
    ...
    async def _assess_functionality(self) -> List[QualityMetric]:
        ...
        try:
            # Test core endpoints
            endpoints = ["/health", "/emotime/status", "/emotime/health", "/metrics"]
    ...
    async def _assess_data_quality(self) -> List[QualityMetric]:
        ...
        try:
            ...
            test_cases = [
                {
                    "text": "I feel absolutely wonderful and happy!",
                    "expected_valence": "high",
                },
                {
                    "text": "This is making me very sad and depressed",
                    "expected_valence": "low",
                },
                {
                    "text": "I'm in a neutral state of mind",
                    "expected_valence": "medium",
                },
                {
                    "text": "I'm extremely excited and energetic!",
            ...
            for case in test_cases:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        print(
                            f"   Emotional analysis test: PASS (conf: {confidence:.2f})"
                        )
                    else:
                        print(
                            f"   Emotional analysis test: FAIL ({response.status_code})"
                        )
                        total_predictions += 1
            ...
            status = (
                "PASS"
                if ml_accuracy > self.quality_thresholds["ml_accuracy"] * 100
                else "FAIL"
            )
            ...
            status = (
                "PASS"
                if avg_confidence > self.quality_thresholds["emotime_confidence"]
                else "FAIL"
            )
    ...
    def _calculate_overall_score(self, metrics: List[QualityMetric]) -> float:
        ...
        for metric in metrics:
            ...
            if metric.status == "PASS":
                ...
                normalized_score = (
                    metric.value / metric.threshold if metric.threshold > 0 else 0.0
                )
            else:
            ...
            if metric.name in ["error_rate_percent"]:
                normalized_score = (
                    1.0 - normalized_score if normalized_score <= 1.0 else 0.0
                )
    ...
    def _generate_recommendations(self, metrics: List[QualityMetric]) -> List[str]:
        ...
        if not recommendations:
            recommendations.append(
                "System quality is within acceptable parameters. Continue monitoring."
            )
        ...
        analysis = {
            ...
            "categories": {
                "performance": [
                    ...
                    for m in metrics
                    if m.name
                    in ["response_time_ms", "throughput_mps", "error_rate_percent"]
                ],
                "functionality": [
                    m
                    for m in metrics
                    if m.name in ["endpoint_availability", "emotime_functionality"]
                ],
                "data_quality": [
                    m
                    for m in metrics
                    if m.name in ["ml_accuracy", "emotime_confidence"]
                ],
                "security": [
                    m for m in metrics if m.name in ["auth_protection", "rate_limiting"]
                ],
            },
        }
