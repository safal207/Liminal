...
class DirectionEncoder:
    ...
    def encode_direction(self, event: NavigationEvent) -> DirectionalVector:
        ...
        try:
            ...
            for direction, patterns in self.direction_patterns.items():
                ...
                keyword_matches = sum(
                    1 for keyword in patterns["keywords"] if keyword in content_text
                )
                ...
                if val_min <= valence <= val_max:
                    ...
                    val_alignment = 1.0 - abs(valence - val_center) / max(
                        abs(val_max - val_center), 0.1
                    )
                ...
                if urg_min <= urgency <= urg_max:
                    ...
                    urg_alignment = 1.0 - abs(urgency - urg_center) / max(
                        abs(urg_max - urg_center), 0.1
                    )
            ...
            primary_dir_str = max(
                direction_scores.keys(), key=lambda k: direction_scores[k]
            )
            primary_direction = SemanticDirection(primary_dir_str)

            # Find secondary direction (second highest if significant)
            sorted_directions = sorted(
                direction_scores.items(), key=lambda x: x[1], reverse=True
            )
            secondary_direction = None

            if (
                len(sorted_directions) > 1
                and sorted_directions[1][1] > self.pattern_threshold
            ):
            ...
            print(
                f"Encoded event '{event.event_type}' -> {primary_direction.value} "
    ...
    def _calculate_stability(self, direction: SemanticDirection) -> float:
        ...
        recent_directions = [
            vector.primary_direction for _, vector in list(self.encoding_history)[-10:]
        ]
...
class MemoryCompass:
    ...
    def create_memory_anchor(
        self,
        anchor_id: str,
        direction_vector: DirectionalVector,
        ...
        try:
            ...
            print(
                f"Memory anchor created: {anchor_id} -> {direction_vector.primary_direction.value}"
            )
            return True
    ...
    def retrieve_direction_from_memory(
        self, context_similarity: float = 0.7
    ) -> Optional[DirectionalVector]:
        ...
        best_anchor = max(self.memory_anchors.values(), key=lambda a: a["access_count"])
...
class TransitionNavigator:
    ...
    def navigate_transition(
        self, from_state: str, to_state: str, current_direction: DirectionalVector
    ) -> TransitionBalance:
        ...
        try:
            ...
            print(
                f"Navigating transition: {transition_id} with {current_direction.primary_direction.value} orientation"
            )
            return balance
            ...
            return TransitionBalance(
                stability_score=0.5, direction_confidence=0.5, transition_readiness=0.5
            )
            ...
            self.transition_history.append(
                {
                    ...
                    "duration": (
                        datetime.now() - transition_data["started_at"]
                    ).total_seconds(),
                    "success": True,
                }
            )
...
class RetrosplenialGateway:
    ...
    def __init__(self):
        ...
        print(
            "Retrosplenial Gateway Layer initialized - Brain's compass with theta oscillations online!"
        )

    async def process_navigation_event(
        self, event: NavigationEvent
    ) -> DirectionalVector:
        """
        ...
        try:
            ...
            theta_modulated_strength, theta_state = (
                self.theta_engine.encode_with_theta_rhythm(
            ...
            print(
                f"Theta modulation: {theta_state.theta_type.value} @ {theta_state.frequency:.1f}Hz "
            ...
            if (
                memory_direction
                and memory_direction.confidence > direction_vector.confidence
            ):
                direction_vector = memory_direction
                print(
                    f"Using memory compass direction: {direction_vector.primary_direction.value}"
                )
            ...
            is_optimal, confidence = (
                self.theta_engine.get_theta_optimal_encoding_window()
            )
            ...
            if self.navigation_context:
                ...
                binding_id, gamma_anchor_strength = (
                    self.gamma_compass.create_synchronized_memory_anchor(
                ...
                coupling_enhanced_strength = (
                    self.gamma_compass.enhance_binding_with_theta_coupling(
                ...
                if self.theta_engine.is_theta_peak():
                    print(
                        "Creating enhanced memory anchor during theta peak with gamma synchrony"
                    )
                    coupling_enhanced_strength *= (
                        1.15  # Additional 15% boost during optimal timing
                    )
                ...
                print(
                    f"Gamma-theta enhanced anchor: {gamma_anchor_strength:.3f} -> {coupling_enhanced_strength:.3f}"
                )
            ...
            print(
                f"Navigation processed: {direction_vector.primary_direction.value} "
    ...
    def get_navigation_analytics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_orientation": (
                {
                    "direction": (
                        self.current_orientation.primary_direction.value
                        if self.current_orientation
                        else None
                    ),
                    "strength": (
                        self.current_orientation.strength
                        if self.current_orientation
                        else 0.0
                    ),
                    "confidence": (
                        self.current_orientation.confidence
                        if self.current_orientation
                        else 0.0
                    ),
                }
            ...
            "theta_oscillations": {
                ...
                "is_optimal_window": self.theta_engine.get_theta_optimal_encoding_window()[
                    0
                ],
