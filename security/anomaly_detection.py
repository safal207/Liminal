    ...
    def extract_features(self, events: List[SecurityEvent]) -> pd.DataFrame:
        ...
        for minute, minute_events in events_by_minute.items():
            ...
            for event in minute_events:
                ...
                if (
                    "status_code" in event.payload
                    and event.payload["status_code"] >= 400
                ):
                    error_count += 1

            avg_payload_size = (
                total_payload_size / len(minute_events) if minute_events else 0
            )
...
class SecurityMonitor:
    ...
    def _cleanup_old_events(self):
        ...
        self.events_buffer = [
            ...
            if current_time - datetime.fromisoformat(event.timestamp)
            <= self.window_size
        ]
        ...
        if any(anomalies):
            ...
            for timestamp in anomaly_times:
                ...
                related_events = [
                    event
                    for event in self.events_buffer
                    if datetime.fromisoformat(event.timestamp).replace(
                ...
                report = {
                    ...
                    "event_types": list(
                        set(event.event_type for event in related_events)
                    ),
                    "events": [
                        {
                ...
                self.logger.warning(
                    f"Anomaly detected! Report: {json.dumps(report, indent=2)}"
                )
