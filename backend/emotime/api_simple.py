...
def translate_to_english(
    ...
    if pipeline is None:
        return (
            text,
            f"Translation skipped: install 'transformers' to enable {src}->en translation.",
            src,
        )
...
def check_usage_limits(user_id: int, plan: str) -> bool:
    limits = {"starter": 1000, "growth": 10000, "enterprise": 100000}
...
async def analyze_emotion(
    request: TextRequest, current_user: Dict = Depends(get_current_user)
):
