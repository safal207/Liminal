...
REQUEST_COUNT = Counter(
    "rgl_requests_total", "Total RGL requests", ["method", "endpoint"]
)
...
NAVIGATION_EVENTS = Counter(
    "rgl_navigation_events_total", "Navigation events processed", ["direction"]
)
...
class NavigationRequest(BaseModel):
    ...
    emotional_valence: float = Field(
        0.0, ge=-1.0, le=1.0, description="Emotional valence (-1 to 1)"
    )
    urgency_level: float = Field(
        0.5, ge=0.0, le=1.0, description="Urgency level (0 to 1)"
    )
...
class RGLCoreService:
    ...
    def _setup_routes(self):
        ...
        async def navigate_event(
            request: NavigationRequest, background_tasks: BackgroundTasks
        ):
            ...
            try:
                if RGL_AVAILABLE:
                    ...
                    response = NavigationResponse(
                        ...
                        gamma_state={
                            "simulation": True,
                            "frequency": random.uniform(30, 100),
                        },
                        processing_time_ms=(time.time() - start_time) * 1000,
                        timestamp=datetime.now().isoformat(),
                    )
                ...
                if self.redis_client:
                    background_tasks.add_task(
                        self._cache_result, request.event_id, response.dict()
                    )

                # Broadcast to WebSocket clients
                await self.connection_manager.broadcast(
                    json.dumps({"type": "navigation_result", "data": response.dict()})
                )
        ...
        async def enter_liminal_state(request: LiminalStateRequest):
            ...
            try:
                result = await self.liminal_system.enter_liminal_state(
                    request.state_id, request.context
                )
                return result
            except Exception as e:
        ...
        if self.redis_client:
            try:
                await self.redis_client.setex(
                    f"rgl:result:{event_id}", 3600, json.dumps(result)  # 1 hour TTL
                )
            except Exception as e:
