...
class ThetaGammaNavigationTester:
    ...
    async def test_theta_modulation(self, event: NavigationEvent) -> Dict[str, Any]:
        ...
        print(
            f"[GAMMA] {gamma_info['gamma_band']} @ {gamma_info['current_frequency']:.1f}Hz"
        )
        print(
            f"[SYNC] Synchrony: {gamma_info['synchrony_strength']:.3f} | Binding: {gamma_info['binding_power']:.3f}"
        )
    ...
    async def test_optimal_timing(self):
        ...
        is_optimal, confidence = (
            self.gateway.theta_engine.get_theta_optimal_encoding_window()
        )
        ...
        if time_to_optimal <= 1.0:
            ...
            # Check again
            is_optimal_after, confidence_after = (
                self.gateway.theta_engine.get_theta_optimal_encoding_window()
            )
            print(
                f"After waiting - optimal: {is_optimal_after}, confidence: {confidence_after:.3f}"
            )
    ...
    async def test_theta_synchronization(self):
        ...
        mock_directions = [
            DirectionalVector(
                primary_direction=SemanticDirection.NORTH, strength=0.8, confidence=0.9
            ),
            DirectionalVector(
                primary_direction=SemanticDirection.EAST, strength=0.6, confidence=0.7
            ),
            DirectionalVector(
                primary_direction=SemanticDirection.WEST, strength=0.4, confidence=0.5
            ),
        ]

        # Synchronize with theta rhythm
        synchronized_directions = (
            self.gateway.theta_engine.get_theta_synchronized_directions(mock_directions)
        )

        print("Original vs Theta-Synchronized Directions:")
        for i, (original, synchronized) in enumerate(
            zip(mock_directions, synchronized_directions)
        ):
            print(f"Direction {i+1}: {original.primary_direction.value}")
            print(
                f"  Original strength: {original.strength:.3f} -> Synchronized: {synchronized.strength:.3f}"
            )
            print(
                f"  Enhancement factor: {synchronized.strength / original.strength:.3f}x"
            )
    ...
    async def test_gamma_memory_binding(self):
        ...
        test_context = {
            ...
            "emotional_context": {"curiosity": 0.8, "focus": 0.9, "confidence": 0.7},
        ...
        binding_id, anchor_strength = (
            self.gateway.gamma_compass.create_synchronized_memory_anchor(
        ...
        query_context = {
            ...
            "emotional_context": {"curiosity": 0.75, "focus": 0.85, "excitement": 0.6},
            "event_content": "Navigate through multi-dimensional awareness space",
            "spatial_elements": ["coordinate_system", "path_optimization"],
        }

        retrieved_binding = self.gateway.gamma_compass.retrieve_gamma_bound_memory(
            query_context
        )
        ...
        print(
            f"  Average binding strength: {gamma_analytics['average_binding_strength']:.3f}"
        )
        print(f"  Average synchrony: {gamma_analytics['average_synchrony']:.3f}")
        print(
            f"  Current binding power: {gamma_analytics['current_binding_power']:.3f}"
        )
    ...
    async def test_theta_gamma_coupling(self):
        ...
        print(
            f"Current Theta: {theta_state.frequency:.1f}Hz (phase: {theta_state.phase:.2f})"
        )
        print(
            f"Current Gamma: {gamma_state.frequency:.1f}Hz ({gamma_state.band.value})"
        )
        ...
        base_strength = 0.8
        enhanced_strength = (
            self.gateway.gamma_compass.enhance_binding_with_theta_coupling(
        ...
        print(
            f"Memory Enhancement: {base_strength:.3f} -> {enhanced_strength:.3f} ({enhancement_factor:.2f}x)"
        )
    ...
    async def run_comprehensive_test(self):
        ...
        print(
            "Features: Theta oscillations + Gamma synchrony + Cross-frequency coupling"
        )
        print(f"{'='*80}")
    ...
    async def print_test_summary(self, coupling_strength: float = 0.0):
        ...
        # Calculate averages
        avg_strength = sum(r["direction_strength"] for r in self.test_results) / len(
            self.test_results
        )
        avg_confidence = sum(
            r["direction_confidence"] for r in self.test_results
        ) / len(self.test_results)
        avg_power = sum(r["theta_power"] for r in self.test_results) / len(
            self.test_results
        )
        avg_processing_time = sum(
            r["processing_time"] for r in self.test_results
        ) / len(self.test_results)
        ...
        if final_analytics["current_orientation"]:
            print(
                f"  Current Orientation: {final_analytics['current_orientation']['direction']}"
            )
            print(
                f"  Orientation Strength: {final_analytics['current_orientation']['strength']:.3f}"
            )

        theta_info = final_analytics["theta_oscillations"]
        print(
            f"  Current Theta: {theta_info['theta_type']} @ {theta_info['current_frequency']:.1f}Hz"
        )
