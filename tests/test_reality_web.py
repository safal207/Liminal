...
def test_reality_web_nodes_edges_and_similarity_and_merge():
    ...
    a = rw.add_node("module_state", traits={"дом": 0.9, "спокойствие": 0.8}, notes=["alpha"])
    b = rw.add_node("module_state", traits={"любовь": 0.7, "страх": 0.2}, notes=["beta"])
    ...
    assert e2.source_id == a.id
    assert e2.target_id == merged.id
