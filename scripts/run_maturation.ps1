...
try:
    ...
    if os.path.exists('$ProjectRoot/tests/test_maturation.py'):
        sys.path.append('$ProjectRoot/tests')
        ...
        # Initialize system
        maturation = ConsciousnessMaturationSystem('$ProjectRoot')
    ...
    if ($result.ExitCode -eq 0) {
        foreach ($line in $result.Output) {
            ...
            elseif ($line -match "\[milestone\]") {
                Write-ColorMessage $line "Philosophy"
            }
...
function Show-Help {
    ...
    Write-ColorMessage "  .\run_maturation.ps1 status" "Info"
    Write-ColorMessage "  .\run_maturation.ps1 monitor -MonitorInterval 30" "Info"
}
